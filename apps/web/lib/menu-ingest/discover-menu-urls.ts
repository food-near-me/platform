...
function isUsefulMenuUrl(url: string, base: URL): boolean {
  ...
  try {
    ...
    return MENU_PATH_PATTERN.test(parsed.pathname) || NAV_MENU_KEYWORDS.test(parsed.pathname);
  } catch {
    return false;
  }
}
 ...
 */
export function discoverMenuUrlsFromHtml(html: string, pageUrl: string): string[] {
  const found: string[] = [];
